...
# Extract TOC pages for analysis
reader_final = PdfReader(output_path)
toc_pages = []
for i in range(min(15, len(reader_final.pages))):
    toc_pages.append(reader_final.pages[i].extract_text())

with open(r"C:\Users\Astrid\toc_llr.txt", "w", encoding="utf-8") as f:
    for i, text in enumerate(toc_pages):
        f.write(f"--- Page {i+1} ---\n{text}\n\n")

print("Extracted TOC to toc_llr.txt")
